...
class StrategyConfig:
    """模組E：每日選股策略（三風格融合）。所有門檻皆為量化詮釋、非保證有效，可調。"""
    # 三風格權重（會自動正規化；改這三個即可調整選股傾向）
    w_value: float = 1 / 3
    w_momentum: float = 1 / 3
    w_dividend: float = 1 / 3
    # 觀測視窗
    momentum_window: int = 63        # 近三個月報酬（約 63 交易日）
    vol_window: int = 60             # 年化波動率取樣天數
    high_window: int = 252           # 52 週高點視窗
    # 價值 / 均值回歸門檻
    per_cheap: float = 10.0          # PER ≤ 此值 → 價值滿分
    per_rich: float = 30.0           # PER ≥ 此值 → 價值 0（亦為「偏貴」警示起點）
    rsi_oversold: float = 30.0       # RSI ≤ 此值 → 超賣（價值滿分）
    rsi_overbought: float = 70.0     # RSI ≥ 此值 → 過熱
    pullback_deep: float = -15.0     # 距季線乖離 ≤ 此值(%) → 跌深滿分
    pullback_none: float = 10.0      # 距季線乖離 ≥ 此值(%) → 無折價
    # 動能 / 趨勢門檻
    ret_weak: float = -10.0          # 近三月報酬 ≤ 此值(%) → 動能 0
    ret_strong: float = 30.0         # 近三月報酬 ≥ 此值(%) → 動能滿分
    high_far: float = 25.0           # 距 52 週高 ≥ 此值(%) → 高點接近度 0
    # 存股 / 防禦門檻
    yield_high: float = 5.0          # 殖利率 ≥ 此值(%) → 殖利率滿分
    vol_low: float = 15.0            # 年化波動 ≤ 此值(%) → 低波動滿分
    vol_high: float = 45.0           # 年化波動 ≥ 此值(%) → 低波動 0
    # 輸出數量
    top_n: int = 12                  # 每日精選候選檔數
    max_warnings: int = 10           # 風險警示最多檔數
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        strategy=StrategyConfig(
            w_value=_get_float("STRATEGY_W_VALUE", 1 / 3),
            w_momentum=_get_float("STRATEGY_W_MOMENTUM", 1 / 3),
            w_dividend=_get_float("STRATEGY_W_DIVIDEND", 1 / 3),
            top_n=_get_int("STRATEGY_TOP_N", 12),
        ),
